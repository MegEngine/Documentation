...
为了利用向量化的特性（避免写出 for 循环），对整个数据 :math:`X` 有：
...
.. figure:: ../../_static/images/gradient-descent-vs.png
   ...
   梯度下降过程中，损失变化的等高线图，箭头表示梯度下降的反向。

* 当 ``batch_size`` 为 1 时，等同于随机梯度下降；
* 当 ``batch_size`` 为 n 时，等同于批梯度下降。

通常我们会根据硬件架构（CPU/GPU）来决定 ``batch_size`` 值的大小，会设置成如 32, 64, 128 等 2 的幂。
...
当加入模型的自变量数量变多，即特征维度越来越高时，我们需要警惕过拟合现象。
过拟合指的是我们的模型在已知数据集上训练表现良好，在新样本上预测却表现不佳的情况。
如果一个模型仅仅在用于训练的数据上表现良好甚至是优异，但却在实际应用时表现不佳...
尴尬，这表明我们训练出的模型已经过拟合，需要极力避免这种情况。
解决过拟合的方法有很多，比如使用更大规模的训练数据集、使用更复杂的模型等等...
本教程中介绍的方法是将数据集进行划分，通常会划分为训练集、验证集与测试集，
在训练的过程中监测过拟合现象是否产生。
...
导致这些情况产生的原因有很多，但本教程的主要目的是帮助你掌握小批量梯度下降的实现，以及模型训练、验证和测试的完整流程。
