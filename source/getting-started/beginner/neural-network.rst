...
值得关注的地方是：激活函数是非线性函数，这也意味着通过多个神经元突触之间的连接，给我们的计算模型引入了非线性。
...
输入层是样本特征，不被记录在神经网络层数内，隐藏层可以有多个，每层的神经元个数需要人为设定。
...
.. seealso::
   ...
   我们解决二分类问题使用的是 Logistic 回归，因此没有采用多层感知机这种称呼。
...
.. dropdown:: :fa:`eye,mr-1` 以这样的方式去编写模型的问题得以体现
...
MegEngine 中的 :mod:`~.module` 模块对经典的网络层级结构和网络模块提供了一类抽象机制，
一切均为 :class:`~.module.Module` 类，除了默认实现了随机初始化策略外，
...
查阅 API 文档可知，计算过程是 :math:`y=xW^T+b`, 与 :func:`~.matmul` 得到的结果是一致的（有浮点误差）：
...
.. seealso::
   ...
   * :class:`~.module.Linear` 作为内置的 :class:`~.module.Module`, 我们不需要对它手动执行初始化过程，
     该过程在其对应的 ``__init__`` 方法中已经实现，允许被用户自己的实现覆盖掉。一些常见的初始化策略在
     :mod:`.module.init` 模块中进行了提供，在本教程中不会进行具体介绍。
...
经过 5 轮训练，通常会得到一个正确率超过 83% 的神经网络模型。
...
.. note::
   ...
   实际上，我们从接触到神经网络模型的那一刻开始，就已经开始进入调参模式了。
