...
区别于数学中的定义，其概念与 NumPy_ 中的 :class:`~numpy.ndarray` 更加相似，
...
其它情况下，想要在 Tensor 中获得某个元素，需要提供对应位置的标量索引（Index），并使用下标运算符 ``[]``:
...
我们通过冒号运算符 ``:`` 进行了切片操作，语法为 ``start:stop:step``, 对应起始索引、终止索引和步长。
...
* 如果下标运算符中没有任何冒号运算符如 ``a[2]``, 则返回与该索引对应的单个元素；
* 如果下标运算符中只有一个冒号运算符，则需要根据参数进行判断：

  * 如果为 ``a[2:]``, 则表明从位置 2 索引开始之后的所有项都被提取；
  * 如果为 ``a[:8]``, 则表明从位置 8 索引开始之前的所有项都被提取；
  * 如果为 ``a[2:8]``, 则表明从位置 2 到 8 索引的所有项将被提取；
...
以表明 MegEngine Tensor 数据结构与 Python 设计的一致性，但实际上二者还是存在着一定的区别。

请猜测一下这段代码的输出，并且尝试运行验证：
...
想要解答这些问题，你必须理解 Tensor 的 :ref:`tensor-fundamental-attributes` 。
